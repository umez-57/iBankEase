...
<head>
  ...
  <title>Staff Withdraw Portal</title>

  <!-- Tailwind CSS via CDN -->
  <script src="https://cdn.tailwindcss.com"></script>

  <!-- JS file for withdraw logic -->
  <script src="staff_withdraw.js" defer></script>
...
<body class="bg-gradient-to-r from-blue-200 to-blue-400 min-h-screen flex items-start justify-center pt-10">
  <!-- Main container: wide card (max-w-5xl) -->
  <div class="bg-white max-w-5xl w-full mx-4 rounded-lg shadow-lg p-6">
    <h1 class="text-2xl font-bold mb-4 text-gray-700">Staff Withdraw Portal</h1>
          ...
          <?php while ($user = mysqli_fetch_assoc($result_users)) : ?>
            ...
            >
              ...
              <td class="py-3 px-4 border border-gray-300 whitespace-nowrap">
                <!-- Withdraw Button -->
                <button
                  onclick="showWithdrawField('<?php echo $user['username']; ?>')"
                  class="bg-blue-600 text-white font-semibold px-3 py-1 rounded
                         transition transform animate-hover hover:bg-blue-700"
                >
                  Withdraw
                </button>
                <!-- Hidden Withdraw Form -->
                <div 
                  id="withdraw-form-<?php echo $user['username']; ?>"
                  class="mt-2 hidden"
                >
                  <input
                    type="number"
                    id="withdraw-amount-<?php echo $user['username']; ?>"
                    placeholder="Enter amount"
                    min="1"
                    class="border border-gray-300 rounded px-2 py-1 
                           focus:outline-none focus:ring-1 focus:ring-blue-400
                           transition-all duration-300 w-36 mr-1"
                  />
                  <button
                    onclick="withdrawAmount('<?php echo $user['username']; ?>')"
                    class="bg-red-600 text-white font-semibold px-2 py-1 rounded
                           transition transform animate-hover hover:bg-red-700"
                  >
                    Confirm Withdraw
                  </button>
                </div>
